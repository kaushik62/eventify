    ...
    <div className="max-w-5xl">
      <div className="mb-8 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">Give your audience a clear reason to show up. You can edit these details later.</p>
        </div>
        <div className="hidden items-center gap-2 text-sm text-muted-foreground sm:flex">
          <MapPin className="h-4 w-4 text-primary" /> Your event, beautifully presented
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1fr_340px]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input required type="time" value={form.eventTime} onChange={(e) => update("eventTime", e.target.value)} />
          </div>
        </div>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium">Ticket price (₹)</label>
            <Input required type="number" min={0} value={form.price} onChange={(e) => update("price", e.target.value)} />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium">Total seats</label>
            <Input required type="number" min={1} value={form.totalSeats} onChange={(e) => update("totalSeats", e.target.value)} />
          </div>
        </div>
        ...
        <aside className="space-y-4">
          <div className="soft-panel overflow-hidden p-3">
            <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-xl bg-muted">
              {imagePreview ? (
                ...
                <div className="px-8 text-center text-muted-foreground">
                  ...
                  <p className="text-sm font-semibold text-foreground">Your event cover</p>
                  <p className="mt-1 text-xs">Use a bright, high-quality image in landscape format.</p>
                </div>
              )}
              {imagePreview && <div className="absolute inset-x-3 bottom-3 rounded-lg bg-black/65 px-3 py-2 text-xs text-white">Preview ready</div>}
            </div>
            ...
            <p className="mt-2 text-center text-xs text-muted-foreground">JPG, PNG or WebP · up to 10 MB</p>
          </div>
          <div className="rounded-xl border border-primary/15 bg-primary/5 p-4 text-sm">
            ...
            <p className="mt-2 leading-6 text-muted-foreground">A specific title, a vivid description, an accurate venue, and a cover image that tells the story at a glance.</p>
          </div>
        </aside>
      </form>
    </div>
